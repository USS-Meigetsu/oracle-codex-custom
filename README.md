...
# Browser multi-turn consult in one ChatGPT conversation (planned follow-ups only)
...
- Conversation mode guidance: use one-shot browser runs for narrow bug reports or quick file-set reviews; use explicit browser follow-ups only for planned turns after the previous answer is complete; use Deep Research for broad public-web questions that need citations. Oracle never invents follow-ups automatically and must not use them to recover a stuck-looking or suspiciously short answer.
...
- Long GPT-5.5 Pro browser consults are normal. Use `--heartbeat`, `oracle status`, and `oracle session <id>` instead of starting a duplicate run if the host agent appears to be waiting. If ChatGPT looks frozen or the answer looks too short, wait longer or reload/re-read the same conversation; do not press ChatGPT's stop button or send a follow-up as recovery.
...
| `--chatgpt-conversation-url <url>`                                             | Resume a specific existing ChatGPT conversation (`https://chatgpt.com/c/...`) in browser mode instead of opening a new chat.                                                                                                                                                                                                              |
...
| `--browser-follow-up <prompt>`                                                 | Browser-only multi-turn consult: submit a planned additional prompt in the same ChatGPT conversation only after the previous answer is complete. Repeat for challenge/revision/final-decision passes. Do not use it to interrupt, continue, or recover a still-generating/short answer. Not supported with Deep Research mode.            |
...
```json5
{
  ...
  browser: {
    ...
    // Optional: pin this project to one existing ChatGPT conversation.
    conversationUrl: "https://chatgpt.com/c/your-project-thread",
...
Use `browser.conversationUrl` (or `browser.resumeConversationUrl`) when a project should keep reusing one existing ChatGPT conversation; project-local `.oracle/config.json` files can point different repositories at different threads. Oracle verifies the conversation history loaded before submitting, so it fails closed instead of silently starting a fresh chat. Model selection still runs before submission unless `--browser-model-strategy current` or `ignore` is used.
